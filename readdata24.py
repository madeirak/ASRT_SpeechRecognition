...
class DataSpeech():
	...
	def LoadDataList(self):
		...
		# 设定选取哪一项作为要使用的数据集
		if(self.type=='train'):
			filename_wavlist_thchs30 = 'thchs30' + self.slash + 'train.wav.lst' #slash斜杠
	...
	def GetData(self,n_start,n_amount=1):
		...
		wavsignal,fs=read_wav_data(self.datapath + filename)  #return 声音信号的时域谱矩阵，帧率Hz(采样率)
	...
	def data_genetator(self, batch_size=32, audio_length = 1600):
		...
		while True:
			...
			for i in range(batch_size):   #遍历每一句
				...
				#y[i] = y[i].T
				#print(i,y[i].shape)
				label_length.append([len(data_labels)]) #label_length列表 统计每句的长度
	...
	def GetSymbolList(self):  #pny字典？ 所有pny的列表
		'''
		加载拼音符号列表，用于标记符号
		返回一个列表list类型变量
		'''
		...
		for i in txt_lines:
			if(i!=''):
				txt_l=i.split('\t')	  #分隔pny和多音字
				list_symbol.append(txt_l[0])   #取pny
		txt_obj.close()
	...
	def SymbolToNum(self,symbol):   #pny2id
		'''
		符号转为数字
		'''
		if(symbol != ''):
	...
	def NumToVector(self,num):  #基于pny字典的独热编码
		'''
		数字转为对应的向量
		'''
		v_tmp=[]
